...
export default function MessengerChat({ currentUser, allUsers = [], initialChat = null }) {
  ...
  function onDrop(e) {
    ...
    if (file && file.type.startsWith('image/')) setStagedFile({ file, previewUrl: URL.createObjectURL(file) });
  }
  ...
  async function reactMsg(id, emoji) {
    ...
    setMessages(prev => prev.map(m => {
      ...
      if (idx > -1) r[emoji].splice(idx, 1);
      else          r[emoji].push(senderEmail);
  ...
  const convSearchLc  = convSearch.toLowerCase();
  const filteredConvs = conversations.filter(c => {
    const u = resolveUser(c.other_email);
    const n = (u?.name || c.other_email || '').toLowerCase();
    return n.includes(convSearchLc) || (c.other_email || '').toLowerCase().includes(convSearchLc);
  });
  ...
  return (
    <div
      className="flex bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden border border-slate-200 dark:border-slate-800"
      style={{ height: 'calc(100vh - 100px)', minHeight: 520 }}
      onDragEnter={onDragEnter} onDragLeave={onDragLeave} onDragOver={onDragOver} onDrop={onDrop}
    >

      {/* ══ LEFT PANEL: Conversations ══════════════════════════════════════════ */}
      <div className="w-72 shrink-0 flex flex-col border-r border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
        ...
        <div className="px-4 pt-5 pb-3 relative">
              ...
              >
                ...
                <div className="max-h-60 overflow-y-auto">
                  {filteredPicker.map(u => (
                    <button key={u.email} onClick={() => { selectChat(u.email); setShowPicker(false); setPickerSearch(''); }}
                      className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors text-left"
                    >
          ...
          <div className="relative">
            ...
            <input value={convSearch} onChange={e => setConvSearch(e.target.value)}
              placeholder="Search…"
              className="w-full pl-9 pr-3 py-2 text-sm bg-slate-100 dark:bg-slate-800 rounded-xl outline-none text-slate-900 dark:text-white placeholder-slate-400"
            />
          </div>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline justify-between gap-1">
                    ...
                    <span className="text-[11px] text-slate-400 shrink-0">{fmtConvTime(conv.created_at)}</span>
                  </div>
      ...
      {!selectedChat ? (
        /* Empty state */
        <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/40 gap-4">
        ...
        <div className="flex-1 flex flex-col min-w-0 relative">
          ...
          {isDragging && (
            <div className="absolute inset-0 z-50 bg-indigo-500/10 border-2 border-dashed border-indigo-400 flex items-center justify-center pointer-events-none">
              <div className="bg-white dark:bg-slate-800 rounded-2xl px-10 py-6 shadow-xl text-center">
          ...
          {/* Chat header */}
          <div className="shrink-0 flex items-center gap-3 px-4 py-3 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
            <UserAvatar user={otherUser} size="md" online={otherStatus.online} />
            <div className="flex-1 min-w-0">
              <p className="font-bold text-slate-900 dark:text-white truncate">{otherUser?.name || selectedChat}</p>
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1 bg-slate-50/80 dark:bg-slate-950/40">
            {groups.map((group, gi) => {
              ...
              return (
                <div key={gi} className={`flex flex-col gap-0.5 ${isMine ? 'items-end' : 'items-start'} mb-1`}>
                  {group.messages.map((m, mi) => {
                    ...
                    return (
                      <div key={m.id}
                        className={`flex items-end gap-2 w-full ${isMine ? 'justify-end' : 'justify-start'}`}
                      >
                        ...
                        {/* Message wrapper */}
                        <div
                          className={`relative group max-w-[68%] ${isMine ? '' : ''}`}
                            ...
                            {hoveredMsg === m.id && !isEditing && (
                              ...
                              >
                                ...
                                <button data-emoji-pick
                                  ...
                                  className="w-7 h-7 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-center text-base transition-colors"
                                  title="React"
                                >😊</button>
                                {/* Reply */}
                                <button onClick={() => { setReplyTo(m); setTimeout(() => inputRef.current?.focus(), 50); }}
                                  className="w-7 h-7 rounded-full hover:bg-slate-100 dark:hover:bg-slate-700 flex items-center justify-center transition-colors"
                                  title="Reply"
                                >
                          ...
                          <AnimatePresence>
                            {showEmojiFor === m.id && (
                              <motion.div data-emoji-pick
                                initial={{ opacity: 0, scale: 0.85, y: 6 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.85 }}
                                transition={{ type: 'spring', stiffness: 320, damping: 22 }}
                                className={`absolute ${isMine ? 'right-0' : 'left-0'} -top-[68px] flex items-center gap-0.5 bg-white dark:bg-slate-800 rounded-full shadow-2xl border border-slate-100 dark:border-slate-700 px-2 py-1.5 z-40`}
                              >
                                {REACTIONS.map(emoji => {
                                  const myReacted = (reactions[emoji] || []).includes(senderEmail);
                                  return (
                                    <button key={emoji} onClick={() => reactMsg(m.id, emoji)}
                                      className={`w-8 h-8 rounded-full flex items-center justify-center text-lg transition-all hover:scale-125 active:scale-110 ${myReacted ? 'bg-indigo-100 dark:bg-indigo-900/40' : ''}`}
                                    >{emoji}</button>
                                  );
                                })}
                              </motion.div>
                          ...
                          {isEditing ? (
                            <div className="flex flex-col gap-2 min-w-[200px]">
                              ...
                              <input value={editText} onChange={e => setEditText(e.target.value)} autoFocus
                                ...
                                className="text-sm px-3 py-2 rounded-xl border border-indigo-300 dark:border-indigo-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white outline-none ring-2 ring-indigo-300 dark:ring-indigo-800 min-w-[200px]"
                              />
                            ...
                            <>
                              ...
                              <motion.div
                                ...
                                className={`
                                  ${bubbleRound}
                                  ${m.attachment_url && !m.content ? 'p-1' : 'px-3.5 py-2.5'}
                                  shadow-sm text-sm
                                  ${isMine
                              ...
                              >
                                {/* Reply preview */}
                                {m.reply_to_id && (
                                  <div className={`mb-2 pl-2.5 border-l-[3px] rounded ${isMine ? 'border-white/40 bg-white/10 pr-2 py-1 rounded-lg' : 'border-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 pr-2 py-1 rounded-lg'}`}>
                                ...
                                {m.attachment_url && (
                                  <img
                                    src={m.attachment_url} alt="Photo"
                                    className={`max-w-[260px] w-full object-cover rounded-xl block cursor-zoom-in ${m.content ? 'mb-2' : ''}`}
                                    onClick={() => setViewerSrc(m.attachment_url)}
                                  />
                                )}
                              ...
                              {isLast && (
                                <div className={`flex items-center gap-1 mt-0.5 px-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                                  ...
                                  {isMine && (
                                    m.is_read
                                      ? <svg className="w-4 h-4 text-indigo-400 shrink-0" viewBox="0 0 20 10" fill="none">
                                      ...
                                      : <svg className="w-3.5 h-3.5 text-slate-400 shrink-0" viewBox="0 0 14 10" fill="none">
          ...
          {/* Context menu (fixed, outside scroll container) */}
          {openMenuId && (() => {
            ...
            return (
              ...
              >
                ...
                {canEdit && (
                  <button
                    onClick={() => { setEditingId(openMenuId); setEditText(msg.content || ''); setEditStagedFile(null); setOpenMenuId(null); setHoveredMsg(null); }}
                    className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors text-left">
                    <svg className="w-4 h-4 text-slate-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                    </svg>
          ...
          <div className="shrink-0 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
            ...
            {replyTo && (
              <div className="flex items-center gap-2 px-4 pt-2.5 pb-0">
                <div className="flex-1 pl-3 border-l-[3px] border-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 pr-3 py-1.5 rounded-lg">
                ...
                <button onClick={() => setReplyTo(null)}
                  className="w-6 h-6 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center text-slate-400 transition-colors shrink-0">
            ...
            {stagedFile && (
              <div className="flex items-center gap-2 px-4 pt-2.5 pb-0">
                <div className="relative">
                  <img src={stagedFile.previewUrl} alt="" className="w-16 h-16 object-cover rounded-xl ring-2 ring-indigo-300" />
            ...
            <div className="flex items-end gap-2 px-3 py-2.5">
              ...
              <button onClick={() => fileRef.current?.click()} title="Send image"
                className="w-9 h-9 shrink-0 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center justify-center text-slate-400 hover:text-indigo-500 transition-colors mb-0.5">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <textarea ref={inputRef} value={newMsg} onChange={handleInputChange} rows={1}
                ...
                placeholder="Message…"
                className="flex-1 bg-slate-100 dark:bg-slate-800 rounded-2xl px-4 py-2.5 text-sm text-slate-900 dark:text-white placeholder-slate-400 outline-none resize-none overflow-y-auto leading-relaxed"
              ...
              <button onClick={sendMsg} disabled={uploading || (!newMsg.trim() && !stagedFile)}
                className={`w-9 h-9 shrink-0 rounded-full flex items-center justify-center mb-0.5 transition-all ${
                  newMsg.trim() || stagedFile
                    ? 'bg-indigo-500 hover:bg-indigo-600 text-white shadow-md hover:shadow-indigo-200 dark:hover:shadow-indigo-900 scale-100'
