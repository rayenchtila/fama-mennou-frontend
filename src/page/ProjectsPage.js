...
const IcShare     = ({s=13}) => <svg width={s} height={s} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>;
...
// ── Share popover — Copy link + WhatsApp + Facebook + Instagram ────────────────
      ...
      <div className="prp-myproject-inner" style={{ padding:'20px 24px' }}>
        ...
        <div className="prp-myproject-toprow" style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16, marginBottom:12 }}>
          ...
          <div className="prp-myproject-right" style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
            {/* Share — copy link / WhatsApp / Facebook / Instagram. Always
                available, locked or not — sharing a project you've already
                awarded is harmless, and there's no reason to hide it. */}
            <div style={{ position:'relative' }}>
              <button ref={shareBtnRef} onClick={()=>setShareOpen(v=>!v)} title={t('prp.share.title')}
              ...
              <AnimatePresence>
                {shareOpen && (
                  <ShareMenu url={shareUrl} title={project.title} onClose={()=>setShareOpen(false)} anchorRef={shareBtnRef} />
                )}
              </AnimatePresence>
            </div>
