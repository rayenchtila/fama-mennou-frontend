      ...
      <div style={{ minHeight: '100vh', background: 'var(--fm-bg)', paddingTop: 100, paddingBottom: 80 }}>
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ textAlign: 'center', marginBottom: 52 }}>
            <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--fm-primary-light)' }}>Support</span>
            <h1 style={{ fontSize: 42, fontWeight: 900, color: 'var(--fm-text-1)', margin: '12px 0 16px', letterSpacing: '-0.02em' }}>Centre d'aide</h1>
            <p style={{ fontSize: 17, color: 'var(--fm-text-5)', lineHeight: 1.8 }}>Trouvez rapidement des réponses à vos questions.</p>
          </div>
            ...
            {FAQS.map((faq, i) => (
              <div key={i} style={{ borderRadius: 16, border: `1px solid ${open === i ? 'rgba(124,108,246,0.35)' : 'var(--fm-border)'}`, background: open === i ? 'rgba(124,108,246,0.07)' : 'var(--fm-surface-hover-soft)', overflow: 'hidden', transition: 'all .2s' }}>
                <button onClick={() => setOpen(open === i ? null : i)}
                  ...
                  <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--fm-text-1)', lineHeight: 1.4 }}>{faq.q}</span>
                ...
                {open === i && (
                  <div style={{ padding: '0 22px 18px' }}>
                    <p style={{ fontSize: 14, color: 'var(--fm-text-5)', lineHeight: 1.8, margin: 0 }}>{faq.a}</p>
                  </div>
                )}
              </div>
            ))}
          ...
          <div style={{ textAlign: 'center', marginTop: 52, padding: '32px', borderRadius: 20, background: 'rgba(124,108,246,0.06)', border: '1px solid rgba(124,108,246,0.15)' }}>
            <p style={{ fontSize: 15, color: 'var(--fm-text-5)', margin: '0 0 16px' }}>Vous n'avez pas trouvé votre réponse ?</p>
