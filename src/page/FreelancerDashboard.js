...
function ProfileTab({ user, updateUser, fetchAccounts }) {
  ...
  function handleSave() {
    ...
    t.textContent = '✅  Modifications sauvegardées !';
    t.style.cssText = 'position:fixed;top:24px;left:50%;transform:translateX(-50%);background:#10b981;color:white;padding:14px 28px;border-radius:12px;font-weight:700;font-size:15px;z-index:2147483647;box-shadow:0 8px 24px rgba(0,0,0,0.4);white-space:nowrap';
    document.body.appendChild(t);
    setTimeout(() => { t.remove(); window.location.reload(); }, 2500);
    // Save bio to DB
              ...
              <div className="absolute right-0 top-12 z-50 w-[min(320px,calc(100vw-2rem))] bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden">
                ...
                <div className="max-h-80 overflow-y-auto">
                  {userNotifs.length === 0 ? (
                    ...
                    <div key={n.id} onClick={() => { markNotificationRead(n.id); setNotifOpen(false); const [base, id] = n.kind.split(':'); if (base === 'course_access') navigate(id ? `/courses/${id}` : '/courses'); }} className={`flex items-start gap-3 px-4 py-3 border-b border-slate-50 dark:border-slate-800/50 cursor-pointer transition-colors last:border-0 group ${n.read ? 'bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/50' : 'bg-indigo-50/60 dark:bg-indigo-900/10 hover:bg-indigo-50 dark:hover:bg-indigo-900/20'}`}>
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 text-sm ${n.kind.startsWith('course_access') ? 'bg-emerald-100 dark:bg-emerald-900/30' : 'bg-indigo-100 dark:bg-indigo-900/30'}`}>
                        {n.kind.startsWith('course_access') ? '🎓' : '🔔'}
                      </div>
                      <div className="flex-1 min-w-0">
