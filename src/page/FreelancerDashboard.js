      ...
      <div className={`${selectedChat ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-72 lg:w-80 border-r border-slate-100 dark:border-slate-800 shrink-0`}>
        ...
        {showPicker && (
          <div className="p-3 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Rechercher..." className="w-full text-xs rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 mb-2" />
      ...
      {selectedChat ? (
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="p-3 border-t border-slate-100 dark:border-slate-800 flex gap-2">
            ...
            <button onClick={sendMsg} disabled={!newMsg.trim()} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-semibold text-sm disabled:opacity-50 transition-colors shrink-0">Envoyer</button>
          </div>
        </div>
      ) : (
